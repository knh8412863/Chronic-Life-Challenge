...
class TestPredictionFlowAPIs(TestCase):
    ...
    async def test_metric_assessment_returns_dyslipidemia_and_obesity_status(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            headers = {"Authorization": f"Bearer {login_response.json()['access_token']}"}
            await client.post("/api/v1/prediction-inputs", json=survey_data, headers=headers)
            lipid_response = await client.post("/api/v1/health/lipid-obesity-records", json=lipid_data, headers=headers)
            assessment_response = await client.get("/api/v1/health/metric-assessments", headers=headers)

        assert lipid_response.status_code == status.HTTP_201_CREATED
        assert assessment_response.status_code == status.HTTP_200_OK
        result = assessment_response.json()["data"]
        assert result["dyslipidemia"]["status"] == "HIGH"
        assert "ldl_cholesterol" not in result["dyslipidemia"]["missing_fields"]
        assert result["obesity"]["status"] == "HIGH"
        assert result["obesity"]["missing_fields"] == []
