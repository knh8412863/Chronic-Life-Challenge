...
import { getPetImage, PET_META } from "../../utils/petAssets";
...
function growthStageHelp(level: number) {
  if (level < 5) {
    return [
      "펫은 총 3단계로 성장합니다.",
      "(Lv.5 - 성장기, Lv.10 - 성체)",
      `현재 다음 단계까지 ${5 - level}레벨 남았습니다.`,
    ];
  }
  if (level < 10) {
    return [
...
export function PetPage({ onNavigate }: PetPageProps) {
  ...
  const stageHelpLabel = stageHelp.join(" ");
    ...
    <div className="page-container">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 14 }}>
        <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: 20, textAlign: "center" }}>
          <div style={{ width: "100%", height: 160, background: "#f5f5f5", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14, fontSize: 64 }}>
            {petImage ? (
              <img src={petImage} alt={`${petMeta.label} ${growthStageLabel(pet.growth_stage)}`} style={{ maxWidth: "90%", maxHeight: "90%", objectFit: "contain" }} />
            ) : petMeta.emoji}
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, margin: "0 0 14px" }}>
            <span style={{ fontSize: 12, color: "#888" }}>
              {petMeta.label} · {growthStageLabel(pet.growth_stage)} 단계
            </span>
            <span className="pet-stage-tooltip" tabIndex={0} aria-label={stageHelpLabel}>
              ?
              <span className="pet-stage-tooltip__bubble" role="tooltip">
                {stageHelp.map((line) => (
